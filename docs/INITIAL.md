...
2. **Ubuntu 24.04+ hosts specifically** restrict unprivileged user namespaces via AppArmor by default, which blocks `bubblewrap` even if `unprivileged_userns_clone` is set. Disable the restriction on the host:
   ...
   **Status (Issue #23, 2026-08-30): fix implemented; live E2E in progress,
   one follow-up fix (Issue #27) pending a host reload.**
   ...
   "Sandbox / security" Lessons Learned for the full live evidence.

   A follow-up live E2E session (same day, host `sudo` access available)
   loaded the profile, pushed all three templates, and confirmed real
   workspace creation succeeds with both `security_opts` actually applied
   (verified via `docker inspect`) — but found the AppArmor profile as
   shipped doesn't actually work due to a rule-precedence bug (Issue #27:
   an unqualified `deny mount,` silently overrode the intended
   `mount options=(make-rslave),` allow, with zero AppArmor audit log
   entry for the resulting denial). Fixed in PR #28 and merged, but that
   fix requires a second `sudo scripts/load-security-profiles.sh` re-run
   on the host to take effect — not yet done as of this note. **Do not
   claim this is fully resolved** until that reload has happened and a
