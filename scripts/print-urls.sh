...
echo "  Coder (workspaces)      http://localhost:${coder_port}  (no seeded login — complete the first-run admin setup in the browser, or 'coder login http://localhost:${coder_port}')"
echo "  Temporal (workflows)    http://localhost:${temporal_ui_port}  (no authentication configured — open access)"
